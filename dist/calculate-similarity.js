...
function parseRawFile(file_path) {
    ...
    const testTitleRegex = /^\s*✓\s+\d+\s+([\w\/.-]+:\d+:\d+)\s+›\s+(.*?)(?:\s+\(([\d.]+)ms\))?$/;
    ...
    if (current_test && (current_test === null || current_test === void 0 ? void 0 : current_test.testSteps.length)) {
        tests.push(current_test);
    }
    console.log(tests);
    return tests;
...
async function evaluateAndReport(test1, test2, similarity_percentage) {
    const [test1_name, test1_steps] = test1;
    const [test2_name, test2_steps] = test2;
    console.log("-===========================");
    const merge_suggestion = await canMerge(test1_steps, test2_steps, similarity_percentage);
    return {
        "Test 1": {
            Name: test1_name,
            Steps: test1_steps,
...
function findFirstMatchIndex(smallerSteps, largerSteps) {
    let matchCount = 0;
...
export default async function calculateSimilarity(logFilePath) {
    try {
        const tests = parseRawFile(logFilePath);
        console.log(tests);
        const filtered_tests = tests.filter((test) => test.testSteps.length >= 5);
        const tests_tuples = filtered_tests.map((test) => [
            test.testId,
            test.testSteps,
        ]);
        ...
        for (let i = 0; i < tests_tuples.length; i++) {
            for (let j = i + 1; j < tests_tuples.length; j++) {
                ...
                console.log("-=== Summarized summarizedTest1Steps:", summarizedTest1Steps);
                console.log("-=== Summarized summarizedTest2Steps:", summarizedTest2Steps);
                const matchCount = findFirstMatchIndex(summarizedTest1Steps, summarizedTest2Steps);
                const similarity_percentage = (matchCount / summarizedTest1Steps.length) * 100;
                console.log(`Similarity between "${test1_id}" and "${test2_id}": ${similarity_percentage.toFixed(2)}%`);
                const reportEntry = await evaluateAndReport([test1_id, summarizedTest1Steps], [test2_id, summarizedTest2Steps], similarity_percentage);
...
//# sourceMappingURL=calculate-similarity.js.map